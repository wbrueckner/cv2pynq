...
def Scharr(src, ddepth, dx, dy, dst=0, scale=1, delta=0, borderType=cv2.BORDER_DEFAULT):
    ...
    if (dst == 0) and (scale == 1) and (delta == 0) and (borderType == cv2.BORDER_DEFAULT) :
        if (src.dtype == np.uint8) and (src.ndim == 2) :
            if (src.shape[0] <= cv2pynq.MAX_HEIGHT) and (src.shape[0] > 0) and (src.shape[1] <= cv2pynq.MAX_WIDTH) and (src.shape[1] > 0) :
                if ((ddepth == -1) and (dx == 1) and (dy == 0)) or ((ddepth == -1) and (dx == 0) and (dy == 1)) :
                    return c.Scharr(src, ddepth, dx, dy)  
    return cv2.Scharr(src, ddepth, dx, dy, dst, scale, delta, borderType)

def Laplacian(src, ddepth, dst=0, ksize=1, scale=1, delta=0, borderType=cv2.BORDER_DEFAULT):
    ...
    if (dst == 0) and (ksize == 1) and (scale == 1) and (delta == 0) and (borderType == cv2.BORDER_DEFAULT) :
        if (src.dtype == np.uint8) and (src.ndim == 2) :
            if (src.shape[0] <= cv2pynq.MAX_HEIGHT) and (src.shape[0] > 0) and (src.shape[1] <= cv2pynq.MAX_WIDTH) and (src.shape[1] > 0) :
                if (ddepth == -1) :
